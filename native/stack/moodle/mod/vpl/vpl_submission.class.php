...
class mod_vpl_submission {
    ...
    public function grade_reduction(&$reduction, &$percent) {
        ...
        $vplinstance = $this->vpl->get_instance();
        if (
            ! ($vplinstance->reductionbyevaluation > 0) ||
        ...
        if ($inst->dategraded > 0) {
            ...
            if ($detailed) {
                ...
                if ($gradinginstance) {
                    ...
                    $ret .= '<div>' .
                            $gradinginstance->get_controller()->render_grade(
                                $PAGE,
                                $this->instance->id,
                                $gradeinfo,
                                $this->instance->grade,
    ...
    public function print_info($autolink = false) {
        ...
        if (
            ($this->vpl->get_instance()->evaluate && ! $this->is_graded()) ||
    ...
    public function savece($result) {
        ...
        $newevaluation = false;
        if (
            $oldce['executed'] == 0 && $result['executed'] > 0
             && ! $this->vpl->has_capability(VPL_GRADE_CAPABILITY)
        ) {
            $newevaluation = true;
        }
