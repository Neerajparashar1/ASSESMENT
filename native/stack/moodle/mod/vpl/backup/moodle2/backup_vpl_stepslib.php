...
require_once(dirname(__FILE__) . '/backup_vpl_filegroup.php');

use mod_vpl\backup\backup_vpl_filegroup;

/**
 * Provide structure of VPL data
 *
 * @copyright 2012 Juan Carlos Rodríguez-del-Pino
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @author Juan Carlos Rodríguez-del-Pino <jcrodriguez@dis.ulpgc.es>
...
class backup_vpl_activity_structure_step extends backup_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $idfield = ['id'];
        $filefields = ['name', 'content', 'encoding'];
        $vpl = new backup_nested_element('vpl', $idfield, $this->vplfields);
        $requiredfiles = new backup_nested_element('required_files');
        $requiredfile = new backup_vpl_filegroup('required_file', $idfield, $filefields);
        $executionfiles = new backup_nested_element('execution_files');
        $executionfile = new backup_vpl_filegroup('execution_file', $idfield, $filefields);
        ...
        $asignedvariation = new backup_nested_element(
            'asigned_variation',
            $idfield,
        ...
        $assignedoverride = new backup_nested_element(
            'assigned_override',
            $idfield,
        ...
        $submissionfile = new backup_vpl_filegroup('submission_file', $idfield, $filefields);
        // Build the tree.
        ...
        $parmvplid = [ 'vpl' => backup::VAR_ACTIVITYID ];
