...
/**
 * Unit tests for the MATLAB tokenizer.
 *
 * @package mod_vpl
 ...
 */
final class tokenizer_matlab_test extends \advanced_testcase {
    /**
     * Test that the MATLAB tokenizer can parse a code example.
     *
...
';
        ...
        $tokenizer = tokenizer_factory::get('matlab');
        $this->assertNotNull($tokenizer, 'Failed to create MATLAB tokenizer');

        $tokens = $tokenizer->parse($code, false);
        $this->assertIsArray($tokens, 'Tokens should be an array');
        $this->assertNotEmpty($tokens, 'No tokens generated for MATLAB');
        ...
        $this->assertTrue($found, 'MATLAB tokenizer should produce at least one expected token type');
    }
