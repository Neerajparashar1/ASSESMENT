...
/**
 * Unit tests for the D tokenizer.
 *
 * @package mod_vpl
 ...
 */
final class tokenizer_d_test extends \advanced_testcase {
    /**
     * Test that the D tokenizer can parse a code example.
     *
...
void main()
{
    ...
    if (x > 0) {
        ...
        $tokenizer = tokenizer_factory::get('d');
        $this->assertNotNull($tokenizer, 'Failed to create D tokenizer');

        $tokens = $tokenizer->parse($code, false);
        $this->assertIsArray($tokens, 'Tokens should be an array');
        $this->assertNotEmpty($tokens, 'No tokens generated for D');
        ...
        $this->assertTrue($found, 'D tokenizer should produce at least one expected token type');
    }
