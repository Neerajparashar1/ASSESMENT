...
/**
 * Unit tests for the Pascal tokenizer.
 *
 * @package mod_vpl
 ...
 */
final class tokenizer_pascal_test extends \advanced_testcase {
    /**
     * Test that the Pascal tokenizer can parse a code example.
     *
...
';
        ...
        $tokenizer = tokenizer_factory::get('pascal');
        $this->assertNotNull($tokenizer, 'Failed to create Pascal tokenizer');

        $tokens = $tokenizer->parse($code, false);
        $this->assertIsArray($tokens, 'Tokens should be an array');
        $this->assertNotEmpty($tokens, 'No tokens generated for Pascal');
        ...
        $this->assertTrue($found, 'Pascal tokenizer should produce at least one expected token type');
    }
