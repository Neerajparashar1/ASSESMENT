...
/**
 * Unit tests for the Go tokenizer.
 *
 * @package mod_vpl
 ...
 */
final class tokenizer_go_test extends \advanced_testcase {
    /**
     * Test that the Go tokenizer can parse a code example.
     *
...
func main() {
    ...
    fmt.Printf("sqrt(2) = %f\n", math.Sqrt(2.0))
        ...
        $tokenizer = tokenizer_factory::get('go');
        $this->assertNotNull($tokenizer, 'Failed to create Go tokenizer');

        $tokens = $tokenizer->parse($code, false);
        $this->assertIsArray($tokens, 'Tokens should be an array');
        $this->assertNotEmpty($tokens, 'No tokens generated for Go');
        ...
        $this->assertTrue($found, 'Go tokenizer should produce at least one expected token type');
    }
