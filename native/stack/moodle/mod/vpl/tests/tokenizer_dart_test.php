...
/**
 * Unit tests for the Dart tokenizer.
 *
 * @package mod_vpl
 ...
 */
final class tokenizer_dart_test extends \advanced_testcase {
    /**
     * Test that the Dart tokenizer can parse a code example.
     *
...
class Example {
  ...
  void display() {
    print("Value: $value");
        ...
        $tokenizer = tokenizer_factory::get('dart');
        $this->assertNotNull($tokenizer, 'Failed to create Dart tokenizer');

        $tokens = $tokenizer->parse($code, false);
        $this->assertIsArray($tokens, 'Tokens should be an array');
        $this->assertNotEmpty($tokens, 'No tokens generated for Dart');
        ...
        $this->assertTrue($found, 'Dart tokenizer should produce at least one expected token type');
    }
