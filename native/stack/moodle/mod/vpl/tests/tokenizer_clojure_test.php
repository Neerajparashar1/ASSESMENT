...
/**
 * Unit tests for the Clojure tokenizer.
 *
 * @package mod_vpl
 ...
 */
final class tokenizer_clojure_test extends \advanced_testcase {
    /**
     * Test that the Clojure tokenizer can parse a code example.
     *
...
(defn -main []
  ...
  (doseq [n numbers]
    (println n)))';
        ...
        $tokenizer = tokenizer_factory::get('clojure');
        $this->assertNotNull($tokenizer, 'Failed to create Clojure tokenizer');

        $tokens = $tokenizer->parse($code, false);
        $this->assertIsArray($tokens, 'Tokens should be an array');
        $this->assertNotEmpty($tokens, 'No tokens generated for Clojure');
        ...
        $this->assertTrue($found, 'Clojure tokenizer should produce at least one expected token type');
    }
