...
/**
 * Unit tests for the Scala tokenizer.
 *
 * @package mod_vpl
 ...
 */
final class tokenizer_scala_test extends \advanced_testcase {
    /**
     * Test that the Scala tokenizer can parse a code example.
     *
...
';
        ...
        $tokenizer = tokenizer_factory::get('scala');
        $this->assertNotNull($tokenizer, 'Failed to create Scala tokenizer');

        $tokens = $tokenizer->parse($code, false);
        $this->assertIsArray($tokens, 'Tokens should be an array');
        $this->assertNotEmpty($tokens, 'No tokens generated for Scala');
        ...
        $this->assertTrue($found, 'Scala tokenizer should produce at least one expected token type');
    }
