...
/**
 * Unit tests for the F# tokenizer.
 *
 * @package mod_vpl
 ...
 */
final class tokenizer_fsharp_test extends \advanced_testcase {
    /**
     * Test that the F# tokenizer can parse a code example.
     *
...
printfn "Factorial of %d is %d" x (factorial x)
...
let distance p1 p2 =
    ...
    Math.Sqrt(dx * dx + dy * dy)';
        ...
        $tokenizer = tokenizer_factory::get('fsharp');
        $this->assertNotNull($tokenizer, 'Failed to create F# tokenizer');

        $tokens = $tokenizer->parse($code, false);
        $this->assertIsArray($tokens, 'Tokens should be an array');
        $this->assertNotEmpty($tokens, 'No tokens generated for F#');
        ...
        $this->assertTrue($found, 'F# tokenizer should produce at least one expected token type');
    }
