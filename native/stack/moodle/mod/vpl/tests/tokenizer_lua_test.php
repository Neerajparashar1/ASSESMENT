...
/**
 * Unit tests for the Lua tokenizer.
 *
 * @package mod_vpl
 ...
 */
final class tokenizer_lua_test extends \advanced_testcase {
    /**
     * Test that the Lua tokenizer can parse a code example.
     *
...
';
        ...
        $tokenizer = tokenizer_factory::get('lua');
        $this->assertNotNull($tokenizer, 'Failed to create Lua tokenizer');

        $tokens = $tokenizer->parse($code, false);
        $this->assertIsArray($tokens, 'Tokens should be an array');
        $this->assertNotEmpty($tokens, 'No tokens generated for Lua');
        ...
        $this->assertTrue($found, 'Lua tokenizer should produce at least one expected token type');
    }
