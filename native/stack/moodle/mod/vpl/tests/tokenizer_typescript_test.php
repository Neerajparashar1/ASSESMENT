...
/**
 * Unit tests for the TypeScript tokenizer.
 *
 * @package mod_vpl
 ...
 */
final class tokenizer_typescript_test extends \advanced_testcase {
    /**
     * Test that the TypeScript tokenizer can parse a code example.
     *
...
class Dog implements Animal {
    ...
    speak(): string {
        return ' . "\x60" . '${this.name} barks.' . "\x60" . ';
    }
}
...
const squares = numbers.map((n: number) => n ** 2);
console.log(squares);
...
';
        ...
        $tokenizer = tokenizer_factory::get('typescript');
        $this->assertNotNull($tokenizer, 'Failed to create TypeScript tokenizer');

        $tokens = $tokenizer->parse($code, false);
        $this->assertIsArray($tokens, 'Tokens should be an array');
        $this->assertNotEmpty($tokens, 'No tokens generated for TypeScript');
        ...
        $this->assertTrue($found, 'TypeScript tokenizer should produce at least one expected token type');
    }
