...
/**
 * JavaScript language similarity class
 *
 * @package mod_vpl
 ...
 * @codeCoverageIgnore
 */
class similarity_javascript extends similarity_generic {
    /**
    ...
    /**
     * Constructor for the JavaScript similarity.
     */
     ...
     *   - Bitwise compound-assignment (&=, |=, ^=, <<=, >>=) → expanded to "= <op>"
     *   - Increment/decrement (++/--) → expanded to "= +"/"-"
     *   - Strict equality (===) → canonical "=="
     *   - Strict inequality (!==) → canonical "!="
     *   - Optional chaining (?.) dropped (noise like this. access)
     *   - Qualified member access (this.) dropped
     *   - Open brackets/parens ([, () dropped (only closers kept)
     *   - Trivial single-statement {}-blocks collapsed
     * Reserved-word normalisations:
     *   - Loop keywords  for / do  → canonical "while"
     *   - Exit keywords  continue  → canonical "break"
            ...
            if ($token->type == token_type::OPERATOR) {
                ...
                switch ($token->value) {
                    ...
                    case ';':
                        ...
                        break;
                    case '===':
                        // Strict equality has same structure as loose equality.
                        $token->value = '==';
                        $ret[] = $token;
                        break;
                    case '!==':
                        // Strict inequality has same structure as loose inequality.
                        $token->value = '!=';
                        $ret[] = $token;
                        break;
                    case '?.':
                        // Optional chaining — same noise as this. qualified access.
                        break;
                    case '.':
