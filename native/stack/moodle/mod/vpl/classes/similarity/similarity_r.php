 ...
 * @codeCoverageIgnore
 */
class similarity_r extends similarity_generic {
    /**
     * Constructor for the R similarity.
     */
     ...
     *   - Left-assignment (<-) → canonical "="
     *   - Right-assignment (->) → canonical "="
     *   - Open brackets/parens [(, [, {) dropped (only closers kept)
     * Reserved-word normalisations:
     *   - Loop keywords  for / repeat  → canonical "while"
     *   - Exit keyword   next (≡ continue)  → canonical "break"
     *
        ...
        foreach ($tokens as $token) {
            if ($token->type == token_type::OPERATOR) {
                switch ($token->value) {
                    ...
                    case '<-':
                    case '->':
                        // Both R assignment forms → canonical =.
                        $token->value = '=';
                        $ret[] = $token;
                        break;
                    default:
