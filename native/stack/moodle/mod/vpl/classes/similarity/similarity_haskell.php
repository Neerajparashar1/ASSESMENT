...
/**
 * Haskell language similarity class
 *
 * @package mod_vpl
 ...
 * @codeCoverageIgnore
 */
class similarity_haskell extends similarity_generic {
    /**
     * Constructor for the Haskell similarity.
     */
    public function __construct() {
        parent::__construct('haskell');
    }
     ...
     *   - "case" (pattern-matching construct) → canonical "if"
     *
    ...
    public function sintax_normalize(&$tokens): array {
        ...
        foreach ($tokens as $token) {
            if ($token->type == token_type::OPERATOR) {
                ...
                switch ($token->value) {
                    case 'case':
                        // Pattern-matching construct → canonical "if".
                        $token->value = 'if';
                        break;
                }
                $ret[] = $token;
