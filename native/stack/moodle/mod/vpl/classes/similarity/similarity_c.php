 ...
 * @codeCoverageIgnore
 */
class similarity_c extends similarity_generic {
    /**
     ...
     *   - Pointer access (->) → rewritten as "( * )."
     *   - Scope (::) and open brackets/parens dropped (only closers kept)
    ...
    public function sintax_normalize(&$tokens): array {
        ...
        foreach ($tokens as $token) {
            if ($token->type == token_type::OPERATOR) {
                if (self::expand_compound_assignment($token, static::COMPOUND_OPS, $ret, $posiniinst)) {
                    $prev = $token;
                    continue;
                }
                switch ($token->value) {
                    ...
                    case ';':
                        ...
                        $posiniinst = count($ret);
                        break;
                    case '->': // Replace "->" by "( * )."; drop "this->" noise.
                        if ($prev->value === 'this') {
                            break;
                        }
