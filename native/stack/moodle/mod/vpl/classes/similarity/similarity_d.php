 ...
 * @codeCoverageIgnore
 */
class similarity_d extends similarity_generic {
    /**
    ...
    /**
     * Constructor for the D language similarity.
     */
     ...
     *   - Power-assignment (^^=) → expanded to "= ^^"
     *   - Increment/decrement (++/--) → expanded to "= +"/"-"
     *   - D not-equal operator (<>) → canonical "!="
     ...
     *   - Loop keywords  for / do / foreach / foreach_reverse  → canonical "while"
            ...
            if ($token->type == token_type::OPERATOR) {
                ...
                switch ($token->value) {
                    ...
                    case ';':
                        ...
                        break;
                    case '<>':
                        // D not-equal (deprecated, same structure as !=).
                        $token->value = '!=';
                        $ret[] = $token;
                        break;
                    case '.':
