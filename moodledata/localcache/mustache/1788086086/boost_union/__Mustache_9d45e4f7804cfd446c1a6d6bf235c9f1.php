...
class __Mustache_9d45e4f7804cfd446c1a6d6bf235c9f1 extends Mustache_Template
{
...
';
        $buffer .= $indent . '    <div class="input-group searchbar w-100" ';
        $blockFunction = $context->findInBlock('searchrole');
        if (is_callable($blockFunction)) {
            $buffer .= call_user_func($blockFunction, $context);
        } else {
            $buffer .= 'role="search" aria-labelledby="searchinput-label-';
            $value = $this->resolveValue($context->find('uniqid'), $context);
            $buffer .= ($value === null ? '' : call_user_func($this->mustache->getEscape(), $value));
            $buffer .= '"';
        }
